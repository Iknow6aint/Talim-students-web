...
import { Loader2, ChevronLeft, Video, Phone, Search, MessageCircle } from "lucide-react";
...
// Utility function to generate consistent colors from strings (same as Teachers app)
...
const GroupChat = ({
  ...
  const resolveSenderInfo = (message: any) => {
    ...
    if (message.senderId && typeof message.senderId === 'object') {
      ...
      console.log('📋 Sender is string:', { senderId, senderName, availableFields: {
        senderName: message.senderName,
  ...
  const transformMessageForUI = (message: any, index: number) => {
    ...
    return {
      ...
      avatar: "/icons/user-placeholder.svg",
    ...
    <div className="w-full h-full flex flex-col bg-white">
      ...
      <div className="flex items-center p-4 border-b border-gray-100 bg-white">
        ...
        <div className="relative flex-shrink-0 mr-3">
          {room.avatarInfo.type === 'image' ? (
            ...
            <div
              ...
              style={{ backgroundColor: room.avatarInfo.bgColor || generateColorFromString(roomName) }}
            >
              {room.avatarInfo.value || roomName.charAt(0).toUpperCase()}
            </div>
          )}
        ...
        <div className="flex-1 min-w-0">
          ...
          {participantsText && (
            <p className="text-sm text-gray-500 truncate">{participantsText}</p>
          )}
        </div>
